...
def format_y_axis(y, pos=None):
    ...
    if y >= 1e12:
        return '{}T'.format(int(y*1e-12))
    elif y >= 1e9:
        return '{}G'.format(int(y*1e-9))
    elif y >= 1e6:
        return '{}M'.format(int(y*1e-6))
    elif y >= 1e3:
        return '{}K'.format(int(y*1e-3))
    else:
        return str(int(y))
...
def render_graph(data, selector="#graph", title_label="", service=None, save_path=None):
    ...
    ax.set_ylim(bottom=df[['bps_in', 'bps_out']].min().min(), top=max(bps_in_peak, bps_out_peak) * 1.1)
    ...
    # Add legend
    legend_elements = [Line2D([0], [0], color='#d1ff27', lw=2, label='Bps IN'),
                       Line2D([0], [0], color='#0d6efd', lw=2, label='Bps OUT'),
                       Line2D([0], [0], color='#6f42c1', lw=2, label='IN Peak: ' + format_y_axis(bps_in_peak)),
                       Line2D([0], [0], color='#20c997', lw=2, label='OUT Peak: ' + format_y_axis(bps_out_peak))]
    ax.legend(handles=legend_elements, facecolor='#191b22', edgecolor='none', labelcolor='#fff', loc='upper center', bbox_to_anchor=(0.5, -0.05), ncol=4, frameon=False)
    ...
    if save_path:
        return fig.savefig(save_path, facecolor=fig.get_facecolor(), dpi=300, edgecolor='none')

    buf = io.BytesIO()
    fig.savefig(buf, format='png', facecolor=fig.get_facecolor(), dpi=300, edgecolor='none')
    buf.seek(0)
